...
import '../mappers/geolocator_position_mapper.dart';
...
class LocationDataSourceImpl implements LocationDataSource {
  ...
  Future<LocationData?> getCurrentLocation() async {
    try {
      ...
      print(
        "【GeolocatorDebug】単発現在地取得成功: Lat=${position.latitude}, Lng=${position.longitude}",
      );
  ...
  Stream<LocationData?> watchLocation() async* {
    ...
    try {
      ...
      try {
        ...
        if (initialPosition != null) {
          print(
            "【GeolocatorDebug】初回取得成功！画面に表示させます: Lat=${initialPosition.latitude}, Lng=${initialPosition.longitude}",
          );
          yield initialPosition.toLocationData(); // ここで画面の「取得中...」が消えて即時反映されます
        }
      } catch (e) {
